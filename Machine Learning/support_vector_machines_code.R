...
data <- read.csv("C:\\Users\\thoma\\OneDrive\\Documents\\ML2\\Module 1\\buad5132-m1-training-data (1).csv")
test <- read.csv("C:\\Users\\thoma\\OneDrive\\Documents\\ML2\\Module 1\\buad5132-m1-test-data (1).csv")
...
library(e1071)
set.seed(150)
split = sample.split(data$TARGET_FLAG, SplitRatio = 0.30)



a = subset(data, split == TRUE)
b = subset(data, split == FALSE)
...
justvariables <- a[ -c(1) ]
justvariablesb<- b[-c(1)]

summary(justvariables)

x <- justvariables
y <- a$TARGET_FLAG
#Convert to data frame
dat <- data.frame(x=x, y=y)
#Fit the SVM with our first linear classifier
...
plot(dat$x.TRAVTIME, dat$y)

table(dat$y, dat$x.Model4Prediction)

dat.te <- data.frame(x=x, y=y)
pred.te <- predict(svmfit, newdata=dat.te)
table(dat$y, dat$x.Model4Prediction)


summary(test)
Prediction<-test["Index","P_TARGET_FLAG"]

justvariablestest<- test[-c(1,2)]
...
#lets use the radial model since it is the best at classifying

summary(test)
justvariablestest<- test[-c(1,2,31)]

x <- justvariablestest
y <- test$P_TARGET_FLAG
#Convert to data frame
dat <- data.frame(x=x, y=y)
#Fit the SVM with our first linear classifier
svmfit <- svm(y~., data=dat, kernel="radial", cost=10, gamma=1)
...
table(dat$y, test$P_TARGET_FLAG)
...
final.svm.model <- svm(y~., data=test.sub, kernel="radial", cost=10, gamma=1)

# Again, double-checking to make sure we don't have any NA's in our Test Data Set
summary(test)
test$TARGET_FLAG <- 0
test.sub <- subset(test, select = c(TARGET_FLAG, AGE , SQRT_TRAVTIME, SQRT_BLUEBOOK, DO_KIDS_DRIVE , URBANICITY,
                                      CLM_FREQ , REVOKED, MVR_PTS, TIF, EDUCATION , MSTATUS, PARENT1 , CAR_USE , CAR_TYPE , JOB, 
                                      HOME_OWNER))
...
########### STAND ALONE SCORING PROGRAM ###############
##### Model coefficients used to create P_TARGET_FLAG
test$P_TARGET_FLAG <- NULL
test$P_TARGET_FLAG <- predict(final.svm.model, newdata = test.sub)
summary(test$P_TARGET_FLAG)
#Prediction File
prediction <- test[c("INDEX","P_TARGET_FLAG")]
write.csv(prediction, file = "C:\\Users\\thoma\\OneDrive\\Documents\\ML2\\Module 1\\prediction_final.csv")
